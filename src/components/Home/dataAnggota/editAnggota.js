...
class editAnggota extends Component {
    ...
    componentDidMount() {
        ...
        this.setState({
            Id_Anggota: navigation.getParam('Id_Anggota', 'NO-ID'),
            username: navigation.getParam('username', 'NO-ID'),
            namaAnggota: navigation.getParam('nama', 'NO-ID'),
            jenisKelamin: navigation.getParam('JenisKelamin', 'NO-ID'),
            tglLahir: navigation.getParam('TanggalLahir', 'NO-ID'),
            alamat: navigation.getParam('alamat', 'NO-ID'),
            noHp: navigation.getParam('Telepon', 'NO-ID'),
            email: navigation.getParam('email', 'NO-ID'),
            jenisAnggota: navigation.getParam('JenisAnggota', 'NO-ID'),
        });
        if (navigation.getParam('JenisKelamin', 'NO-ID') == "Laki-Laki") {
            this.setState({ checked: 0 });
        }
        else if(navigation.getParam('JenisKelamin', 'NO-ID') == "Perempuan") {
            this.setState({ checked: 1 });
        }
        //{this.state.jenisKelamin == 'Perempuan' ? this.setState({ checked: 1 }) : this.setState({ checked: 0 })}
    }
    updateDataAnggota = () => {
        this.setState({ ActivityIndicator_Loading: true }, () => {
            fetch('http://dewoksatria.000webhostapp.com/sipuja/updateAnggota.php',
                {
                    method: 'POST',
                    headers:
                        {
                    ...
                    Alert.alert(
                        ...
                        [
                            {
                                text: 'OK', onPress: () => this.props.navigation.navigate('detailAnggota')
                            },
                        ],
                        { cancelable: false }
                    )
                ...
                <ScrollView>
                    <KeyboardAvoidingView behavior="padding">
                        <Content contentContainerStyle={{
                            ...
                            <TouchableOpacity
                                onPress={this.updateDataAnggota}
                                activeOpacity={0.5}
